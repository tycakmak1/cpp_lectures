...
|11|`&&`|Logical `AND`[^3]|Left-to-right|
|12|`\|\|`|Logical `OR`[^3]|Left-to-right|
|13|`?:`[^4]|Ternary conditional|Right-to-left|
...
[^5]: Comma Operator is a binary operator that evaluates its first operand and discards the result, it then evaluates the
...
|2|`a++` `a--`<br>`type(a)` `type{a}`<br>`a()`<br>`a[]`<br>`a.b` `a->b`|Suffix/postfix increment and decrement<br>Functional cast<br>Function call<br>Subscript<br>Member access|Left-to-right|
|3|`++a` `--a`<br>`+a` `-a`<br>`!a` `~a`<br>`(type)a`<br>`*a`<br>`&a`<br>`sizeof`[^1]<br>`co_await`<br>`new` `new[]`<br>`delete` `delete[]`|Prefix increment and decrement<br>Unary plus and minus<br>Logical `NOT` and bitwise `NOT`<br>C-style cast<br>Indirection (dereference)<br>Address-of<br>Size-of<br>await-expression<br>Dynamic memory allocation<br>Dynamic memory deallocation|Right-to-left|
...
|7|`a << b`[^2] `a >> b`[^2]|Bitwise left shift and right shift|Left-to-right|
...
|14|`a && b`|Logical `AND`[^3]|Left-to-right|
|15|`a \|\| b`|Logical `OR`[^3]|Left-to-right|
|16|`a ? b : c`[^4]|Ternary conditional|Right-to-left|
