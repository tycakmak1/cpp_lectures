...
|2|`++` `--`<br>`+` `-`<br>`!` `~`<br>`(type)`<br>`*`<br>`&`<br>`sizeof` [^sizeof]<br>`_Alignof`|Prefix increment and decrement<br>Unary plus and minus<br>Logical `NOT` and bitwise `NOT`<br>Cast<br>Indirection (dereference)<br>Address-of<br>Size-of<br>Alignment requirement|Right-to-left
|3|`*` `/` `%`|Multiplication, division and remainder|Left-to-right|
|4|`+` `-`|Addition and subtraction|Left-to-right|
|5|`<<` `>>`|Bitwise left shift and right shift|Left-to-right|
|6|`<` `<=`<br><br>`>` `>=`<br><br>|Relational operators for smaller than and smaller than or equal to respectively<br>Relational operators for bigger than and bigger than or equal to respectively|Left-to-right|
...
5.  `>>`(Bitwise Right Shift)<br>
    `<<`(Bitwise Left Shift)
    ##### ###REMARK###
    >   Right operand of bit shift operators must be smaller than the number of bits of its left operand, also must be a
    >   positive value.
